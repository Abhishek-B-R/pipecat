...
def _judge(answers, explainer=None, **kwargs) -> tuple[EvalJudge, _FakeClassifier]:
    ...
    return EvalJudge(classifier=classifier, explainer=explainer, **kwargs), classifier
...
class TestJudgeOverAnLLM(unittest.IsolatedAsyncioTestCase):
    """The judge given an LLM service classifies and explains with it."""
    ...
    async def test_a_no_is_explained_by_the_same_llm(self):
        llm = _FakeLLMService(
            [
    ...
    async def test_a_classifier_of_its_own_judges_without_reasons(self):
        llm = _FakeLLMService([json.dumps({"verdict": {"choice": "no", "probabilities": {}}})])
        judge = EvalJudge(classifier=LLMClassifier(llm=llm))
        judge.add_assistant_message("Hello there.")
        verdict = await judge.evaluate("describes the weather")
        self.assertEqual(verdict.verdict, "no")
        self.assertEqual(len(llm.calls), 1)

    def test_a_judge_needs_a_service_or_a_classifier(self):
        with self.assertRaises(ValueError):
            EvalJudge()
        with self.assertRaises(ValueError):
            EvalJudge(_FakeLLMService([]), classifier=_FakeClassifier([]))
